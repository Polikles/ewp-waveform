...
Target architecture: **AnalysisFrame** → static **VisualField** → style geometry → **RenderPlan** → effects/encoder. RenderPlan picks MASK_FAST (coverage + downstream color) or RGBA_2D (general/reference) from required capabilities, not from the style name alone. The current monochrome filled ribbon with glow uses MASK_FAST; particles/gradients fall back to RGBA_2D. Glow/particles stay downstream. Old recentered and even/odd fold outputs remain reference modes. New center-out field: 65 slots, one visual center (`scripts/spectrum_field_ribbon.py`). Ribbon raster defaults to 2× supersample (`RIBBON_SUPERSAMPLE`); scroll stays 12×. Analysis is a clip-length `AnalysisSequence` (NumPy `rfft`, PCM loaded once).
